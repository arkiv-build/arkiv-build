...
- GOOD examples: "instagram_like_mvp_acme_7x9k", "voting_app_arkivlabs_qz3p1", "kanban_demo_solo_9m2v"
- BAD examples: "instagram", "instagram_like_mvp", "instagram_like_mvp_v1", "app", "myapp", "0xabc...-instagram_mvp"
  - Why bad: the first four are generic enough that anyone could pick them, causing public-DB collisions. The last one violates the no-wallet-prefix rule.
...
export const buildDataModelUserPrompt = ({
  ...
  const sharedRunRequirements = [
      'Project attribute naming requirement for this run: every entity must include an indexed attribute named exactly "project" whose value follows the shape <appKind>_<orgSlug>_<randomSuffix> (example: "twitter_like_mvp_acme_7x9k"). The slug must be globally unique across the public Arkiv network — never use bare names like "twitter", "myapp", or "twitter_like_mvp_v1", and never prefix with a wallet address. Invent a short org/author slug and a 4–6 char random alphanumeric suffix if the user has not supplied one.',
...
export const buildDataModelEvaluatorUserPrompt = ({
  ...
  [
    ...
    '- Critical violation if the project attribute value is generic / globally non-unique. Specifically reject values that: (a) contain only a single segment ("instagram", "myapp"); (b) contain only an app-kind without any org slug or random suffix ("instagram_like_mvp"); (c) end in version markers like "_v1", "_v2" instead of a random suffix; (d) omit a 4–6 char random alphanumeric suffix. The slug must contain at least 3 underscore-separated segments AND end with a 4–6 char alphanumeric suffix. Generic org segments like "demo", "solo", "test", "acme" ARE acceptable when paired with a real random suffix — do NOT reject these. A passing value looks like "instagram_like_mvp_acme_7x9k" OR "coursera_style_learning_demo_n4x7q".',
    ...
    '- Critical violation if the candidate model is missing entities the canonical reference app for this category clearly requires. Apply this reasoning: name the dominant reference app the user is describing (YouTube for video sharing, Coursera for LMS, Spotify for music streaming, GitHub for dev platform, etc.). Mentally list the tables a traditional SQL backend would have. If a clearly-canonical entity is missing (e.g., video sharing without WatchHistory or Subscription, LMS without graded Submission when assessments are implied, music app with playlists but no PlaylistTrack-with-position), REJECT. Do not enforce arbitrary minimum entity lists — enforce the canonical reference shape.',
